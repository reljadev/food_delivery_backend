...
public class OrderService {
	...
	public void addOrder(Parameters parameters) {
		Order o = new Order();
		o.setStatus(Status.DELIVERED);
		o.recordTimeOfExecution();
		o.setRestaurant(restaurantRepository
							.findById(parameters.getRestaurantId())
							.orElseThrow(() -> {
	...
	private void addMealToOrder(Order order, String mealId, Integer mealQuantity) {
		...
		if(!meal.getRestaurant().equals(order.getRestaurant())) {
			throw new ResponseStatusException(
					  HttpStatus.NOT_FOUND, 
					  "meal " + meal.getId() + " is not offered in specified restaurant");
		}
	...
	// because I don't want to pass whole objects (meal, restaurant and user) in request
//	public void updateOrder(String id, Order o) {
//		Order oSaved = orderRepository.findById(id).orElse(null);
//		if(oSaved != null) {
//			oSaved.setStatus(o.getStatus());
//			orderRepository.save(oSaved);
//		}
//	}
